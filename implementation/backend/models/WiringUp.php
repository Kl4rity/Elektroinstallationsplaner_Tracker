...
class WiringUp implements iPostRequestExecutor {
    ...
    public function executeRequest($requestData) {
        ...
        if(empty($devices[0]["name"])){
            $deivces = null;
        }

        if(empty($fuses[0]["name"])){
            $fuses = null;
        }

        return array("currentLevel" => "wiring", "nextLevel" => "NONE", "data" => $devices, "fuses" => $fuses);
    }
